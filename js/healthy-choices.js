 ...
 require(['jquery','vizjs_toolkit/core/visualizeHelper'], function($,VisualizeHelper) {	
	...
	$('.seasonalLegend_reset').on('click', function(e) {
		resetControls('SupplyTime');
		e.preventDefault();
	});	

	$('.seasonalLegend').on('click', function(e) {
		var filterVal = $(this).data( "valueFilter" );
		passControls('SupplyTime', filterVal);
		e.preventDefault();
	});		
	...
	*/
	vizInstance.done(function(v) {
		masterReport = renderReportLink(master, '#healthy-choices', v);
		updateProduct('Ebony Red Pepper', '2', 'Fresh Vegetables', '2.88', '14');
	});	   
		
	function renderReportLink(uri, container, vizInstance) {
		 
		return vizInstance.report({
			resource: uri,
			container: container,
